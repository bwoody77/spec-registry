...
component Checkbox(
  ...
  button {
    ...
    // `text-align: start` is load-bearing, and its absence was invisible for
    ...
    // and only logical values are accepted, so `left` is an error. Vector's
    // ToggleRow carries the identical two lines for the identical reason.
    text(label) {
      style: labelType
      weight: 500
